...
const selectFinance = (state: any) => state.finance;
...
export const selectTotalExpenses = createSelector(
  ...
  expenses => {
    ...
    return expenses.reduce(
      (acc: number, expense: ExpensesIncomeWithLogsData) =>
        acc + expense.amount,
      0
    );
  }
...
export const selectTotalIncome = createSelector([selectIncome], income => {
  ...
  return income.reduce(
    (acc: number, singleIncome: ExpensesIncomeWithLogsData) =>
      acc + singleIncome.amount,
    0
  );
});
...
export const selectPercentageExpenses = createSelector(
  ...
  (expenses, totalExpenses) =>
    expenses?.map((expense: ExpensesIncomeWithLogsData) => ({
      ...expense,
...
export const selectPercentageIncome = createSelector(
  ...
  (income, totalIncome) =>
    income?.map((singleIncome: ExpensesIncomeWithLogsData) => ({
      ...singleIncome,
...
export const selectLatestExpenses = createSelector(
  ...
  expenses => {
    ...
    const latestMonthExpenses = [] as ExpensesIncomeWithoutPercentData[];

    expenses?.forEach((expense: ExpensesIncomeWithLogsData, index: number) => {
...
export const selectLatestIncome = createSelector([selectIncome], income => {
  ...
  const currentMonth = currentDate.getMonth();
  const currentYear = currentDate.getFullYear();
  const latestMonthIncome = [] as ExpensesIncomeWithoutPercentData[];

  income?.forEach((singleIncome: ExpensesIncomeWithLogsData, index: number) => {
...
export const selectAllTimeExpensesCategoriesId = createSelector(
  ...
  expensesLogs => {
    const categoriesIds = expensesLogs?.map(
      (expenseLog: SingleExpenseIncomeLogData) => expenseLog.categoryId
    );
...
export const selectAllTimeExpensesCategories = createSelector(
  ...
  (categoriesIds, expensesLogs, expenses) => {
    const zeroValueCategories = expenses?.filter(
      (expense: ExpensesIncomeWithLogsData) =>
        !categoriesIds.includes(expense.id)
    );
    ...
    const valuableCategories = categoriesIds?.map(categoryId => {
      const lastOccurence = expensesLogs
        ...
        .reverse()
        .findIndex(
          (entry: SingleExpenseIncomeLogData) => entry.categoryId === categoryId
        );
      ...
      const expenseTotalAmount = expensesLogs?.reduce(
        (acc: number, entry: SingleExpenseIncomeLogData) => {
...
export const selectAllTimeIncomeCategoriesId = createSelector(
  [selectIncomeLogs],
  incomeLogs => {
    const categoriesIds = incomeLogs?.map(
      (incomeLog: SingleExpenseIncomeLogData) => incomeLog.categoryId
    );
...
export const selectAllTimeIncomeCategories = createSelector(
  ...
  (categoriesIds, incomeLogs, income) => {
    const zeroValueCategories = income?.filter(
      (singleIncome: SingleExpenseIncomeLogData) =>
        !categoriesIds.includes(singleIncome.id)
    );
    ...
    const valuableCategories = categoriesIds?.map(categoryId => {
      ...
      const incomeTotalAmount = incomeLogs?.reduce(
        (acc: number, entry: SingleExpenseIncomeLogData) => {
          if (entry.categoryId === categoryId) {
            return acc + entry.amount;
          } else {
